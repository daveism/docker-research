load_data <- function(dpath, opath, f_prfx, yrs2proc) {
  ...
  for (I in 1:length(yrs2proc)) {
    ...
    ras <- lapply(files2proc, raster) # List of function calls
    ras.stack <- stack(ras) # Symblic object pointing to raster files
    output[a:b,] <- as.matrix(ras.stack, nrow=np, ncol=spy) # Populate array
  }
  cat(' Done\n')
