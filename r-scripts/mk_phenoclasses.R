...
dprefix <- Sys.getenv('DATA_FILE_PREFIX') # prefix used in data filenames
dpath <- Sys.getenv('DATA_IN_FOLDER')
opath <- Sys.getenv('DATA_OUT_FOLDER')
tpath <- Sys.getenv('DATA_TEMPLATE_FOLDER')
ipath <- Sys.getenv('DATA_CACHE_FOLDER')
spath <- Sys.getenv('SCRIPTS_PATH')
tmpfilename <- Sys.getenv('DATA_CACHE_FILENAME_PREFIX')

YEAR_START <- Sys.getenv('YEAR_START')
YEAR_END <- Sys.getenv('YEAR_END')
yrs2proc <- seq(YEAR_START, YEAR_END) # Calendar years of data to process
...
big.x <- load_data(dpath=dpath, opath=ipath, f_prfx=dprefix,
                   tmpfilename=tmpfilename, yrs2proc=yrs2proc)
...
library(rgdal)
library(raster)
file.list <- list.files(tpath, pattern='template')	# place where input data live (modify 'pattern' as needed)
r <- raster(paste(tpath,file.list[1], sep="/"))	# use first one as template
...
print(file.path(opath,"kmoutput.RData"))
saveRDS(km, file=file.path(opath,"kmoutput.RData"))
